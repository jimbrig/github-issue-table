...
```{r repos}
jimbrig_repos <- gh::gh("/users/jimbrig/repos", .limit = Inf) %>%
  purrr::map_depth(1, purrr::pluck, "full_name") %>%
  purrr::map_chr(pluck, 1)

# add tychobra repos
tychobra_repos <- gh::gh("/orgs/tychobra/repos", .limit = Inf) %>%
  purrr::map_depth(1, purrr::pluck, "full_name") %>%
  purrr::map_chr(purrr::pluck, 1) %>%
  # filter out any repos with numbers
  purrr::discard(stringr::str_detect, regex("[:digit:]"))

repos <- c(jimbrig_repos, tychobra_repos)
```
...
```{r, download}
# Download all issues
jimbrig_issues <- all_issues(jimbrig_repos) %>%
  # Combine issue_url and title
  mutate(title = glue('<a href="{issue_url}">{title}</a>')) %>%
  select(-issue_url)

tychobra_issues <- all_issues(tychobra_repos) %>%
  # Combine issue_url and title
  mutate(title = glue('<a href="{issue_url}">{title}</a>')) %>%
  select(-issue_url)
```
